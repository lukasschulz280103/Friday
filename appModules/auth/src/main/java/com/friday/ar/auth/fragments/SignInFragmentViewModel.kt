...
class SignInFragmentViewModel(private val context: Context) : ViewModel() {
    companion object {
    ...
    private val createNewUserCompleteListener = OnCompleteListener<AuthResult> { task ->
        ...
        if (task.isSuccessful) {
            ...
            AnalyticUtil.logUserCreation(context)
        } else {
            if (task.exception != null) {
                AnalyticUtil.logFailedUserCreation(context, null, task.exception!!.message)
    ...
    private val onSignInCompletionListener = OnCompleteListener<AuthResult> { task ->
        ...
        if (task.isSuccessful) {
            notifyAuthCompleted()
            AnalyticUtil.logSuccessfulAuthentication(context, Constant.AnalyticEvent.LOGIN_EVENT_EMAIL)
        } else {
            try {
                throw task.exception!!
            } catch (e: FirebaseAuthEmailException) {
                AnalyticUtil.logFailedAuthentication(context, Constant.AnalyticEvent.LOGIN_EVENT_EMAIL, e.errorCode, e.message)
                ...
                AnalyticUtil.logFailedAuthentication(context, Constant.AnalyticEvent.LOGIN_EVENT_EMAIL, null, e.message)
            }
        }
    }
        ...
        firebaseAuth.signInWithCredential(credential)
                .addOnCompleteListener { task ->
                    ...
                    if (task.isSuccessful) {
                        ...
                        val jobInfo = JobInfo.Builder(Constant.Jobs.JOB_SYNC_ACCOUNT, ComponentName(context, AccountSyncService::class.java))
